        ...
        <div class="m-6 mb-12 rounded-xl p-6 shadow-xl sm:p-10">
            <form action="{{ route('Userprofile') }}" method="post">
                ...
                <div class="grid gap-6 mb-6 md:grid-cols-2">
                    ...
                    <div>
                        @error('tel')
                            <p class="text-red-600">{{ $message }}</p>
                        @else
                            <label for="tel" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Номер телефона</label>
                        @enderror
                    ...
                    <div>
                        @error('website')
                            <p class="text-red-600">{{ $message }}</p>
                        @else
                            <label for="website" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Страница сайта</label>
                        @enderror

                        <input type="url" id="website" name="website" value="{{ $data->website }}"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            placeholder="SeoFast.ru">
                    </div>
                    <div>
                        @error('visitors')
                            <p class="text-red-600">{{ $message }}</p>
                        @else
                            <label for="visitors" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Уникальный просмотр</label>
                        @enderror
                ...
                <div class="mb-6">
                    ...
                    @else
                        <label for="email"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Электронная почта</label>
                    @enderror
                ...
                <div class="flex items-start mb-6">
                    ...
                    <label for="remember" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Принимаю условия <a href="{{ route('accept') }}" target="_blank"
                            class="text-blue-600 hover:underline dark:text-blue-500">ознакомился</a>.</label>
                </div>
                <button type="submit"
