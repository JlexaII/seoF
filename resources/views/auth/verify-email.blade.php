...
@section('content')
    ...
    <main class="flex flex-col justify-center p-6 pb-12">
        <div class="mx-auto max-w-md">
            ...
            <h2 class="mt-2 text-2xl font-bold text-gray-900 sm:mt-6 sm:text-3xl">Подтвердите электронную почту</h2>
        </div>
        <div class="mx-auto mt-6 w-full max-w-md rounded-xl bg-white/80 p-6 shadow-xl backdrop-blur-xl sm:mt-10 sm:p-10">
            ...
            <form action="{{ route('verification.send') }}" method="post" autocomplete="off">
                ...
                <div>
                    <button type="submit"
                        class="flex w-full items-center justify-center rounded-md bg-green-600 py-2 px-4 font-semibold text-white shadow-lg transition 
                        duration-150 ease-in-out hover:bg-green-700 hover:shadow-xl focus:shadow-xl focus:outline-none focus:ring-2 focus:ring-green-500 
                        focus:ring-offset-2">Отправить ссылку для восстановления</button>
                </div>
            </form>
        </div>
    </main>
